...
public class PrincipalTask2 {
	...
	public static void main(String[] args) {
		...
		PrincipalTask2 main = new PrincipalTask2();
		try {
	...
	private void testePoolThread() throws InterruptedException {
		 ...
		 */
		/**
		 * Interrompe as threads necessario o run estar envolvido em um try - catch
		 *
		 * pool.shutdownNow();
		 *
		 */
		...
		/**
		 * Ele so ira fazer o shutdown quando nao tiver mas nenhuma tarefa rodando
		 */
	...
	public void testTimeOut() {
		...
		executor.schedule(new Runnable() {
			public void run() {
				handler.cancel(PrincipalTask2.this.isRunning);
			}
